function objToStr(obj) {
	...
	//  id=zhangsan&name=sex&password=123
}
				...
				function($rootScope, $scope, $state, AuthService, UserService,
					...
					$scope.login = function(uname, pwd) {
						...
						var data = {
							username : uname,
							password : pwd,
							signUuid : $scope.uuid,// 验证码对应的uuid
							signCode : $scope.captcha//输入的验证码
						};
				...
				function($rootScope, $location, $scope, $state, $interval,
										...
										if (res.status == 1) {
										  $scope.username_error_msg ="";
										   $scope.nameIsPass = true;
											...
											$scope.username_error_msg =hmd.presentStatus(70);
											$scope.nameIsPass = false;
										}
									});
						} else {
					...
					$scope.nextone = function() {
						if (!($scope.nameIsPass && $scope.phoneIsPass
								...
								&& $scope.isPassCaptcha && $scope.invitIsPass)) {
							$scope.yzusername();
							$scope.yzphone();
							$scope.yzinviteid();
							$scope.authCaptcha();
...
//											if (response.status == 1) {
												...
												var data = {
													...
													userInviteid : $scope.userInviteid,
													origin : 'PC'
												};
						...
						PostService.authIdentity(objToStr(data)).success(
								function(res) {
									if (res.status == 1) {
										...
										_btn.removeAttr('disabled');
									}else if(res.status == 1111){
										
										hmd.popupErrorInfo('查无此人', 'error');
										_btn.removeAttr('disabled');
									}
				...
				function($scope, $state, AuthService, PostService, UserService,
					...
					$scope.payAccount = function() {
						...
						var token = AuthService.getToken();
						 ...
						 var str='token='+ token+'&money='+money;
//						 alert(str);
//						
					window.location.href="http://localhost:8080/pay/index.jsp?"+str
//					
