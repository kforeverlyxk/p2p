...
public class BigDecimalUtil {
	 ...
	 */
	public static BigDecimal getOldMoneyAndInterest(double transferredMoney, double expectedannual,
			double transferredperiod) {
	...
	/**
	 * 将10日内充值的金额与充值后投资的金额进行对比， 若前者大于后者，则可提现金额应为当前余额-10日内充值的金额+投资金额；
	 * 若前者小于后者，则可提现金额=当前余额
	 * 
	 * @param balance
	 * @param chargeMoney
	...
	/*
	 * public static BigDecimal getExtract(String balance,String cost){
	 * System.out.println("-----sysout ---balance"+balance); BigDecimal bal=new
	 * BigDecimal(balance); BigDecimal co=new BigDecimal(cost);
	 * if(bal.compareTo(co)==-1 || bal.compareTo(co)==0){ BigDecimal flag=new
	 * BigDecimal("-1"); System.out.println("flag---->"+flag); return flag;
	 * }else{ BigDecimal sub = bal.subtract(co);
	 * System.out.println("sub-----"+sub); return sub; }
	 * 
	 * }
	 */
	...
	public static String monthWinInterest(String account, String yearInterest) {
		...
		String monthWinInterestStr = String.valueOf(format2decimal(accountNum.multiply(
				yearInterestNum.divide(BigDecimalUtil.numConstant("12", "100"), 6, BigDecimal.ROUND_HALF_UP))));
	...
	public static String monthWin(String account, String yearInterest) {
		BigDecimal accountNum = new BigDecimal(account);
		BigDecimal yearInterestNum = new BigDecimal(yearInterest);
		// String monthWinInterestStr =
		// accountNum.multiply(yearInterestNum).divide(BigDecimalUtil.numConstant("12",
		// "100"),2,BigDecimal.ROUND_DOWN).toString();
		String monthWinInterestStr = String.valueOf(format2decimal(accountNum.multiply(
				yearInterestNum.divide(BigDecimalUtil.numConstant("12", "100"), 6, BigDecimal.ROUND_HALF_UP))));
	...
	public static String expectWinMoney(String account, String endMonthNum, String yearInterest) {
		...
		// String expectWinMoney =
		// accountM.multiply(yearInterestNum).multiply(endMonth).divide(BigDecimalUtil.numConstant("100",
		// "12"),2,BigDecimal.ROUND_HALF_UP).toString();
		// String expectWinMoney =
		// String.valueOf(format2decimal(accountM.multiply(yearInterestNum.multiply(endMonth).divide(BigDecimalUtil.numConstant("100",
		// "12"),6,BigDecimal.ROUND_HALF_UP))));
		String expectWinMoney = String.valueOf(format2decimal(accountM.multiply(yearInterestNum
				.divide(BigDecimalUtil.numConstant("100", "12"), 6, BigDecimal.ROUND_HALF_UP).multiply(endMonth))));
	...
	public static String expectDeposit(String account, String endMonthNum, String yearInterest) {
		...
		BigDecimal yearInterestNum = new BigDecimal(yearInterest);
		BigDecimal endMonth = new BigDecimal(endMonthNum);
		String expectWinMoney = accountM.multiply(yearInterestNum).multiply(endMonth)
				.multiply(endMonth.add(new BigDecimal(1)))
				.divide(BigDecimalUtil.numConstant("100", "24"), 2, BigDecimal.ROUND_HALF_UP).toString();
		;
		System.out.println("expectWinMoney....." + expectWinMoney);
		return expectWinMoney;
	}
	...
	/**
	 * 
	 * 方法描述：月存代收利息（新投资） 创建人：wangcan 创建时间：2015年3月28日 上午11:15:22 修改人：wangcan
	 * 修改时间：2015年3月28日 上午11:15:22 修改备注：
	 * 
	 * @version 1.0
	 * @param account
	 *            购买金额
	...
	public static String monthDepositInterestCollection(String account, String yearInterest, String endMonthNum) {
		...
		BigDecimal yearInterestNum = new BigDecimal(yearInterest);
		BigDecimal endMonth = new BigDecimal(endMonthNum);
		String monthDepositInterestCollection = accountM.multiply(
				yearInterestNum.divide(new BigDecimal(100)).divide(new BigDecimal(12), 6, BigDecimal.ROUND_HALF_UP))
				.multiply(endMonth).toString();
		return monthDepositInterestCollection;
	}

	/**
	 * 
	 * 方法描述：月乘代收利息（新投资） 创建人：wangcan 创建时间：2015年3月28日 上午11:16:51 修改人：wangcan
	 * 修改时间：2015年3月28日 上午11:16:51 修改备注：
	 * 
	 * @version 1.0
	 * @param account
	 *            购买金额
	...
	public static String takeTheCollectionOfMonthlyInterest(String account, String endMonthNum, String yearInterest) {
		...
		BigDecimal yearInterestNum = new BigDecimal(yearInterest);
		BigDecimal endMonth = new BigDecimal(endMonthNum);
		String expectWinMoney = accountM.multiply(
				yearInterestNum.divide(new BigDecimal(100)).divide(new BigDecimal(12), 6, BigDecimal.ROUND_HALF_UP))
				.multiply(endMonth).toString();
		return expectWinMoney;
	}
	 ...
	 * takeTheCollectionOfMonthlyInterest("100", "3", "8.1");
	...
	/**
	 * 
	 * 方法描述：月取代收利息（新投资） 创建人：wangcan 创建时间：2015年3月28日 上午11:18:17 修改人：wangcan
	 * 修改时间：2015年3月28日 上午11:18:17 修改备注：
	 * 
	 * @version 1.0
	 * @param account
	 *            购买金额
	...
	/**
	 * 
	 * 方法描述：月取到期应会总本息 创建人：wangcan 创建时间：2015年4月8日 下午3:10:04 修改人：wangcan
	 * 修改时间：2015年4月8日 下午3:10:04 修改备注：
	 * 
	 * @version 1.0
	 * @param account
	 *            购买金额
	 ...
	 * @param yearInterest
	 *            预期年化利率
	 * @param MonthlyExtractInterest
	 *            每月提取利息
	 * @return
	 *
	 */
	public static String endInvestTotalMoney(String account, String endMonthNum, String yearInterest,
			String monthlyExtractInterestNum) {
		BigDecimal accountM = new BigDecimal(account);
		BigDecimal yearInterestNum = new BigDecimal(yearInterest).divide(new BigDecimal(12), 6,
				BigDecimal.ROUND_HALF_UP);
		BigDecimal monthlyExtractInterest = new BigDecimal(monthlyExtractInterestNum);
		BigDecimal expectwinmoney = accountM
				.multiply((new BigDecimal("1").add(yearInterestNum.divide(new BigDecimal(100))))
						.pow(Integer.parseInt(endMonthNum)))
				.subtract(monthlyExtractInterest
						.multiply(((new BigDecimal("1").add(yearInterestNum.divide(new BigDecimal(100))))
								.pow(Integer.parseInt(endMonthNum))).subtract(new BigDecimal("1")))
						.divide(yearInterestNum.divide(new BigDecimal(100)), 6, BigDecimal.ROUND_HALF_UP))
				.add(monthlyExtractInterest.multiply(new BigDecimal(endMonthNum)));
	...
	/**
	 * 最大可提现金额计算公式 最大提现金额 = 历史充值累计 – 历史投资成功金额 + 历史投资回款金额 – 历史累计提现金额 – 用户明下付款中金额
	 * - 用户名下投资申请金额
	 * 
	 * @return
	 */
	...
	/*
	 * public static String getMaxExtractMoney(double d1,double d2,double
	 * d3,double d4,double d5,double d6){
	 * 
	 * BigDecimal b1 = new BigDecimal(Double.toString(d1)); BigDecimal b2 = new
	 * BigDecimal(Double.toString(d2)); BigDecimal b3 = new
	 * BigDecimal(Double.toString(d3)); BigDecimal b4 = new
	 * BigDecimal(Double.toString(d4)); BigDecimal b5 = new
	 * BigDecimal(Double.toString(d5)); BigDecimal b6 = new
	 * BigDecimal(Double.toString(d6)); String maxExtractMoney =
	 * b1.subtract(b2).add(b3).subtract(b4).subtract(b5).add(b6).toString();
	 * return maxExtractMoney; }
	 */

	/**
	 * 
	 * //rate*months+rate/maxDaysOfMonth*天数 accountD*(rateD/100)*months
	 * 
	 * @return
	 */
	/**
	 * 计算 月 利息lbb,针对提取结清，老债权（给吴国青用） //accountD*(rateD/100)*months
	 * 
	 * @param d1
	 *            : 投资的金额
	 * @param d2
	...
	public static double getMonthRateInvest(double d1, double d2, int d3) {
		...
		BigDecimal b2 = new BigDecimal(Double.toString(d2));
		BigDecimal b3 = new BigDecimal(Double.toString(d3));
		double monthRateInvestD = format2decimal(
				b1.multiply(b2.divide(new BigDecimal(100), 6, BigDecimal.ROUND_HALF_UP)).multiply(b3));
	...
	public static double getDayRateInvest(double d1, double d2, int d3, int d4) {
		...
		BigDecimal b4 = new BigDecimal(Double.toString(d4));
		double monthRateInvestD = format2decimal(
				b1.multiply(b2.divide(new BigDecimal(100)).divide(b3, 6, BigDecimal.ROUND_HALF_UP)).multiply(b4));
		return monthRateInvestD;
	}
