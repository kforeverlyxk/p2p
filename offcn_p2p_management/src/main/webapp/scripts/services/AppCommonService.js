var option = {};
option.url = {
	'api_admin' : '/offcn_p2p_action/admin/',
	'api_account' : '/offcn_p2p_action/account/',
	'api_user' : '/offcn_p2p_action/user/',
	'api_accountrecord' : '/offcn_p2p_action/accountrecord/',
	'api_product' : '/offcn_p2p_action/product/',
	'api_creditor' : '/offcn_p2p_action/creditor/',
	'api_match' : '/offcn_p2p_action/matchManagement/',
	'api_message' : '/offcn_p2p_action/',
	'api_UsersFunds' : '/offcn_p2p_action/UsersFunds/'
};
option.header = {
	'headers' : {
		...
		.factory(
				...
				function($http) { //将$http注入，就可以向服务器发送请求
					return {
						signIn : function($str) {
							// 向action发送请求
							/*option.url.api_account + 'login  请求路径
							 * $str  请求参数
							 * option.header 请求头
							 */
							return $http.post(option.url.api_account + 'login',
									$str, option.header);
							// /offcn_p2p_action/account/login
						}
					}
				})
				...
				function($http) {
					return {
						...
						getProductList : function() {
							...
							// /offcn_p2p_action/product/findAllProduct
						},

						getProductById : function($str) {
							return $http.post(option.url.api_product
									+ 'findProductById', $str, option.header);
							// /offcn_p2p_action/product/findProductById
						},
						// 根据产品来获取利率信息
				...
				function($http) {
					return {
						...
						entryDebet : function($str) {
							...
							// /offcn_p2p_action/creditor/addCreditor
						}
					}
				})
